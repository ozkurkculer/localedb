      ...
      <p className="text-sm text-muted-foreground">
        ...
        {(search || selectedContinent) && (
          <span className="text-foreground/60">
            {" "}
            / {countries.length}{" "}
            {t("countries.filters.continent.all").toLowerCase()}
          </span>
        )}
      </p>
  ...
  return (
    <Link href={`/countries/${country.code}`} className="group relative block h-full">
      ...
      >
        ...
        <h3
          className={`relative mb-2 line-clamp-1 text-base font-bold bg-gradient-to-br ${style.nameGradient} bg-clip-text text-transparent ${style.nameGradientHover} transition-all`}
        >
          {country.name}
        </h3>
