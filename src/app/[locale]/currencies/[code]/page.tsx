...
export async function generateMetadata({ params }: CurrencyPageProps): Promise<Metadata> {
    ...
    try {
        ...
        ogUrl.searchParams.set(
            'subtitle',
            `Symbol: ${currency.data.symbol} • Used in ${currency.data.countries.length} countries`
        );
...
export default async function CurrencyPage({ params }: CurrencyPageProps) {
    ...
    const jsonLd = {
        ...
        url: `https://localedb.org/currencies/${currency.data.code}`,
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {usedIn.map((country) => (
                                <Link
                                    ...
                                    className="group flex items-center gap-3 rounded-md border border-border/40 bg-background/50 p-3 transition-colors hover:border-primary/50 hover:bg-muted/50"
                                >
